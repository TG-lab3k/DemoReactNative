...
async function startDiscover() {
  console.log('开始搜索打印机');
  var discoverResult = await eposPrinterDiscovery.discover({
    findFirstAndroid: true,
    usbSerialNumber: true,
    scanningTimeoutAndroid: 900000,
  });
...
export default function EposPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    var discoverySubscription = eventEmitter.addListener(
      'onDiscoveryDone',
      async (params: any) => {
        console.log('打印机搜索成功！！！');
        for (let deviceInfo of params) {
          console.log(
            `#onDiscoveryDone# usbSerialNumber: ${deviceInfo.usbSerialNumber}, target: ${deviceInfo.target}, usb: ${deviceInfo.usb}, DeviceName: ${deviceInfo.name}`,
          );
